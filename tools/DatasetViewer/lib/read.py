...
def load_velodyne_scan(file):
    ...
    #print("lidar_read ", scan.shape)
    if scan.size % 5 == 0:
        return scan.reshape((-1, 5))  # [:, :4] DEFAULT
    # elif scan.size % 4 == 0:
    #     return scan.reshape((-1, 4))  # [:, :4]
    # elif scan.size % 3 == 0:
    #     return scan.reshape((-1, 3))  # [:, :3]
    else:
        raise ValueError(f"Unexpected scan size: {scan.size}")
...
def load_calib_data(path_total_dataset, name_camera_calib, tf_tree, velodyne_name='lidar_hdl64_s3_roof'):
    ...
    important_translations = [velodyne_name, 'radar_ars300', cam_name]
    translations = []

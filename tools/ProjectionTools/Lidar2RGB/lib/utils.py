...
def project_pointcloud(lidar, vtc, velodyne_to_camera, image_shape, init=None, draw_big_circle=False):
    ...
    def py_func_create_lidar_img(lidar_points_2D, lidar_points, img_width=1248, img_height=375, init=None):
        ...
        print(f"  Image bounds: width=[0, {img_width}), height=[0, {img_height})")
        print(f"  Input points: {lidar_points_2D.shape[0]}")
        print(f"  Points within width bounds: {within_image_boarder_width.sum()}")
        print(f"  Points within height bounds: {within_image_boarder_height.sum()}")
        print(f"  Points within both bounds: {valid_points.sum()}")
        if valid_points.sum() > 0:
            print(f"  2D projection ranges: x=[{lidar_points_2D[:, 0].min():.1f}, {lidar_points_2D[:, 0].max():.1f}], y=[{lidar_points_2D[:, 1].min():.1f}, {lidar_points_2D[:, 1].max():.1f}]")

        values = lidar_points[:, coordinates]
        if init is None:
            #image = -120.0 * np.ones((img_width, img_height, 3))
            image = -120.0 * np.ones((img_height, img_width, 3))
        else:
            image = init  # Keep shape as (H, W, 3)

        img_coordinates = lidar_points_2D[coordinates, :].astype(dtype=np.int32)

        if not draw_big_circle:
            image[img_coordinates[:, 1], img_coordinates[:, 0], :] = values.transpose()
        else:
            # Elementwise circle drawing through opencv
            len_pts = img_coordinates.shape[0]
    ...
    def py_func_lidar_projection(lidar_points_3D, vtc, velodyne_to_camera, shape, init=None):  # input):
        ...
        img_width = shape[1]
        img_height = shape[0]
        print(f"\n--- LiDAR Projection Debug ---")
        print(f"Input points: {lidar_points_3D.shape[0]}")
        ...
        print(f"Points after min_x > {min_x} filter: {valid.sum()}")
        if valid.sum() > 0:
            print(f"  3D X range: [{lidar_points_3D[valid, 0].min():.2f}, {lidar_points_3D[valid, 0].max():.2f}]")
            print(f"  3D Y range: [{lidar_points_3D[valid, 1].min():.2f}, {lidar_points_3D[valid, 1].max():.2f}]")
            print(f"  3D Z range: [{lidar_points_3D[valid, 2].min():.2f}, {lidar_points_3D[valid, 2].max():.2f}]")

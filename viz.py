...
def _batch_modalities(modalities, device):
    """Wrap a single sample's modality dict into the list-of-dicts format model.forward expects."""
    return [{k: v.to(device) for k, v in modalities.items()}]


@torch.no_grad()
def find_frames_with_predictions(
    model, dataset, device, score_threshold=0.3, min_predictions=1, eval_head="all"
):
    """
    ...
    for idx in tqdm(range(len(dataset)), desc="Searching frames"):
        modalities, _, _ = dataset[idx]
        batch = _batch_modalities(modalities, device)
        all_detections = model(batch)
        head_key = eval_head if eval_head in all_detections else list(all_detections.keys())[0]
        outputs = all_detections[head_key][0]
...
def visualize_predictions_grid(
    ...
    for sample_idx, ds_idx in enumerate(indices):
        ...
        modalities, target, _ = dataset[ds_idx]
        batch = _batch_modalities(modalities, device)
        all_detections = model(batch)
        head_key = eval_head if eval_head in all_detections else list(all_detections.keys())[0]
        outputs = all_detections[head_key][0]
